...
class YearPicker @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
   ...
   // var currentYearList = mutableListOf<Int>()
    ...
    val root= inflate(context, R.layout.fokin_year_picker,this)
        ...
        plusPeriod.setOnClickListener { currentPeriod=Pair(currentPeriod.first+step,currentPeriod.second+step) }
        minusPeriod.setOnClickListener { currentPeriod=Pair(currentPeriod.first-step,currentPeriod.second-step) }

        fun setOutYear(tv:TextView){
            outYear=(tv.text).toString().toInt()
        }

        yearListTextView.forEach { textView-> textView.setOnClickListener { setOutYear(textView) } }
    }
